...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train) :
      for j in range(num_test):
        difference=x_train_flat[i]-x_test_flat[j]
        dists[i,j]=torch.sum(difference*difference)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #we can write (a-b)^2 = a^2 +b^2 -2ab
    train_sq = torch.sum(x_train_flat ** 2, dim=1).view(-1, 1)  # (num_train, 1)  a^2
    test_sq = torch.sum(x_test_flat ** 2, dim=1).view(1, -1)    # (1, num_test)    b^2

    #dot product (ab term)
    cross_term = torch.matmul(x_train_flat, x_test_flat.T)      # (num_train, num_test)
    dists=train_sq+test_sq -2*cross_term
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
      dist_j=dists[:,j]
      _,idx=torch.topk(dist_j,k,largest=False)  #shape (k,)
      closest_y=y_train[idx]
      votes = torch.bincount(closest_y, minlength=y_train.max().item() + 1)
