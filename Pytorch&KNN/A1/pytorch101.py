...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Create range from first to stop, stepping by 10
    values = list(range(first, stop + 1, 10))
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    x[0:2,2:6]=2
    x[0:2,1:2]=1
    x[2:4, 0:2] = torch.tensor([3, 4])
    x[2:4, 2:4] = torch.tensor([3, 4])
    x[2:4, 4:6] = torch.tensor([5, 5])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    len_y=max(x)+1
    len_x=len(x)
    y = torch.zeros(len_x,len_y,dtype=torch.int64)
    y[torch.arange(len_x), x] = 1
    ##########################################################################
    #                      TODO: Implement this function                     #
    ##########################################################################
    # Replace "pass" statement with your code
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y=x.clone()
    y[torch.arange(x.size(0)), row_min_idx] = 0
    ##########################################################################
    #                      TODO: Implement this function                     #
    ##########################################################################
    # Replace "pass" statement with your code
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mu = x.sum(dim=0) / M
    x_centered = x - mu
    var = (x_centered ** 2).sum(dim=0) / (M - 1)
    std = var.sqrt()
    y = x_centered / std
    ##########################################################################
    #                      TODO: Implement this function                     #
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    cat_xs=torch.cat(xs)
    segment_sizes = torch.repeat_interleave(torch.arange(len(xs), device=ls.device), ls)
    sums = torch.zeros(len(xs), dtype=cat_xs.dtype, device=cat_xs.device).index_add(0, segment_sizes, cat_xs)
    y = sums / ls.to(cat_xs.dtype)
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    sorted_vals, sorted_idx = x.sort(stable=True)
    neq = torch.ones_like(sorted_vals, dtype=torch.bool)
    neq[1:] = sorted_vals[1:] != sorted_vals[:-1]
    uniques = sorted_vals[neq]
    indices = sorted_idx[neq]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
